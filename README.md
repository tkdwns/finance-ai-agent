...
## 🔄 5인 에이전트 순차 처리 워크플로우 (Sequential Process Flow)

```mermaid
flowchart TD
    Start["💬 사용자 질의 입력 (CLI / Web UI)"] --> Step1
    
    subgraph Pipeline ["👥 5인 금융 AI Agent 자율 워크플로우"]
        Step1["🕵️ 1단계: 자료 조사관 (Researcher)<br/>DART·주가·금리·부동산 수집"] --> Step2["📊 2단계: 수치 분석관 (Analyst)<br/>Cross-Asset 파급 효과 정량 계산"]
        Step2 --> Step3["⚖️ 3단계: 팩트 검증관 (Compliance)<br/>원문 대조 & 환각(오류) 100% 차단"]
        Step3 --> Step4["✍️ 4단계: 보고서 작성관 (Writer)<br/>통합 마크다운 리포트 작성"]
        Step4 --> Step5["🧐 5단계: 수석 검수관 (Critic)<br/>품질 검수 & Self-Correction"]
        Step5 -- "개선 필요 시 재작성 피드백" --> Step4
    end
    
    Step5 --> End["📊 최종 검증 보고서 및 Audit Trail 저장"]
```

---

## 🏗️ 아키텍처 구조도 (Architecture Diagram)
...
## 📋 에이전트 처리 단계별 핵심 역할 및 담당 업무

| 단계 | 에이전트 이름 | 비전공자를 위한 핵심 역할 설명 | 주요 수집 및 처리 데이터 |
| :---: | :--- | :--- | :--- |
| **1단계** | **🕵️ 자료 조사관 (Researcher)** | DART 공시, 실시간 주가, 부동산 실거래가, 국채 금리 등 필요한 원천 데이터를 직접 수집함 | DART, Yahoo Finance, FRED, 국토교통부 |
| **2단계** | **📊 수치 분석관 (Analyst)** | 금리 변동이 주식과 부동산 대출에 미치는 파급 효과(Cross-Asset)를 정량 계산함 | PER/PBR, 주담대 금리, 한미 금리차 |
| **3단계** | **⚖️ 팩트 검증관 (Compliance)** | 수집된 숫자와 공시 원문을 대조하여 AI의 거짓말(환각 현상)을 100% 차단 및 검증함 | 원문 대조, 팩트체크 가드레일 |
| **4단계** | **✍️ 보고서 작성관 (Writer)** | 검증된 결과를 바탕으로 읽기 쉬운 깔끔한 마크다운 분석 보고서를 작성함 | 통합 마크다운 리포트, 요약 표 |
| **5단계** | **🧐 수석 검수관 (Critic)** | 작성된 보고서를 다각도로 검토하고, 품질이 미흡하면 고쳐 쓰게 만든 뒤 승인함 | Self-Correction, 최종 품질 검수 |
